		...
		<div class="row form-inline" align="left" style="margin-top:10px;">
			<div class="col-md-12">
				老人姓名： <input type="text" class="form-control" id="oldmanname"
					...
					class="form-control" id="hospital1" placeholder="请输入医院名称">
				医生姓名： <input type="text" class="form-control" id="doctor1"
					placeholder="请输入医生姓名">
			...
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary"
						id="save_healthrecord_btn">保存</button>
				</div>
			</div>
		</div>
	</div>
		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
							...
							<form id="formid">
								<table class="table table-bordered table-hover text-center">
									...
									<tr>
										...
										<td>相关禁忌事项：</td>
										<td colspan="2"><textarea class="form-control" rows="2"
												name="tabooltems" id="select_tabooltems"></textarea></td>
									</tr>
								</table>
							</form>
						</div>
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
					<button type="button" class="btn btn-primary"
						id="update_healthrecord_btn">更新</button>
				</div>
			</div>
		</div>
	</div>
							...
							<form id="formid">
								<table class="table table-bordered table-hover text-center">
									...
									<tr>
										...
										<td>就诊时间：</td>
										<td><p class="form-control-static"
												id="select_clinicaltime1"></td>
										<td>复诊时间：</td>
										<td><p class="form-control-static"
												id="select_appointmenttime1"></td>
									</tr>
									<tr class="active">
										<td class="">主诉内容：</td>
										<td colspan="2"><p class="form-control-static"
												id="select_medicalrecord11"></td>
										<td>体格检查内容：</td>
										<td colspan="2"><p class="form-control-static"
												id="select_chekcontent1"></td>
									</tr>
									<tr>
										<td>辅助检查内容：</td>
										<td colspan="2"><p class="form-control-static"
												id="select_ancillarycheckcontent1"></td>
										<td>实际检查内容：</td>
										<td colspan="2"><p class="form-control-static"
												id="select_realcheckcontent1"></td>
									</tr>
									<tr class="active">
										<td>正常使用药物：</td>
										<td colspan="2"><p class="form-control-static"
												id="select_normalusedrug1"></td>
										<td>本次诊断结果：</td>
										<td colspan="2"><p class="form-control-static"
												id="select_diagnosis1"></td>
									</tr>
									<tr>
										<td>治疗处方和保健处方：</td>
										<td colspan="2"><p class="form-control-static"
												id="select_treatmentcareprescription1"></td>
										<td>相关禁忌事项：</td>
										<td colspan="2"><p class="form-control-static"
												id="select_tabooltems1"></td>
									</tr>
								</table>
							</form>
						</div>
	...
	<SCRIPT type="text/javascript">
		...
		function build_page_nav(result) {
			...
			if (result.extend.pageInfo.hasPreviousPage == false) {
				...
				prePageLi.click(function() {
					go(result.extend.pageInfo.pageNum - 1, hospital1,
							oldmanname, doctor1);
				});
			}
			...
			lastPageLi
					.click(function() {
						go(result.extend.pageInfo.pages, hospital1, oldmanname,
								doctor1);
					});
					...
					.each(
							...
							function(index, item) {
								...
								var olderliveinfo = $("<td></td>").append(
										ChangeDateFormat(item.clinicaltime));
								var olderservice = $("<td></td>").append(
										ChangeDateFormat(item.appointmenttime));
								...
								$("<tr></tr>").append(checkBoxTd).append(
										...
										oldersex).append(olderphone).append(
										olderidcar).append(olderliveinfo)
										.append(olderservice).append(btnTd)
										.appendTo("#table_data");
							});
		}
		...
		$("#selectwith").click(
				function() {
					...
					go(1, $("#hospital1").val(), $("#oldmanname").val(), $(
							"#doctor1").val());
							...
							layer
									.confirm(
											...
											function(index) {
												...
												$
														.ajax({
															url : "${APP_PATH}/health/deleteallhealthrecord/"
																	+ del_idstr,
															type : "DELETE",
															success : function(
																	result) {
		...
		$(document).on(
				...
				function() {
					...
					//3.把老人的id传递给模态框的编辑按钮
					$("#update_healthrecord_btn").attr("edit-id",
							$(this).attr("edit-id"));
		...
		$(document)
				.on(
						...
						function() {
							...
							$
									.ajax({
										url : "${APP_PATH}/health/selecthealthrecord/"
												+ serch,
										type : "GET",
										...
										success : function(result) {
											...
											if (result.code == 100) {
												...
												$("#select_name1").text(
														rel.oldman.name);
												//alert(rel.oldman.name);
												$("#select_hospital1").text(
														rel.hospital);
												$("#select_department1").text(
														rel.department);
												$("#select_doctor1").text(
														rel.doctor);
												$("#select_clinicaltime1")
														.text(
																ChangeDateFormat(rel.clinicaltime));
												$("#select_appointmenttime1")
														.text(
																ChangeDateFormat(rel.appointmenttime));
												$("#select_medicalrecord11")
														.text(
																rel.medicalrecord1);
												$("#select_chekcontent1").text(
														rel.chekcontent);
												$(
														"#select_ancillarycheckcontent1")
														.text(
																rel.ancillarycheckcontent);
												$("#select_realcheckcontent1")
														.text(
																rel.realcheckcontent);
												$("#select_normalusedrug1")
														.text(rel.normalusedrug);
												$("#select_diagnosis1").text(
														rel.diagnosis);
												$(
														"#select_treatmentcareprescription1")
														.text(
																rel.treatmentcareprescription);
												$("#select_tabooltems1").text(
														rel.tabooltems);
											}
										}
									});
