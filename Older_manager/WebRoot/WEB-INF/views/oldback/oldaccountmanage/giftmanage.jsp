...
<body style="margin: 15px;">
		...
		<div class="row" style="margin-top:5px">
			<div class="col-md-12">
				<form class="form-inline"
					action="${APP_PATH}/giftmanagement/queryAllGift.action"
					method="post">
					...
					<div class="form-group">
						<label>礼品编号</label> <input type="text" name="giftnumber"
							class="form-control" placeholder="请输入礼品编号">
					</div>
					<div class="form-group">
				...
				<nav aria-label="Page navigation">
					<ul class="pagination">
						...
						<c:if test="${pageInfo.hasPreviousPage==true}">
							<li><a
								href="${APP_PATH}/giftmanagement/queryAllGift.action?pn=${1}">首页</a>
							</li>
							<li><a
								href="${APP_PATH}/giftmanagement/queryAllGift.action?pn=${pageInfo.pageNum-1}">
						...
						<c:forEach items="${pageInfo.navigatepageNums}" var="p">
							<c:if test="${p==pageInfo.pageNum}">
								<li class="active"><a
									href="${APP_PATH}/giftmanagement/queryAllGift.action?pn=${pageInfo.pageNum}">${p}</a></li>
							</c:if>
							<c:if test="${p!=pageInfo.pageNum}">
								<li><a
									href="${APP_PATH}/giftmanagement/queryAllGift.action?pn=${p}">${p}</a></li>
							</c:if>
						</c:forEach>
						...
						<c:if test="${pageInfo.hasNextPage!=false}">
							...
							<li><a
								href="${APP_PATH}/giftmanagement/queryAllGift.action?pn=${pageInfo.pages}">末页</a>
							</li>
						</c:if>
					</ul>
				</nav>
			...
			<div class="modal-content">
					...
					<form id="add_gift_form">
						<table class="table-striped table-bordered text-center">
							<tr>
								...
								<td><input type="text" id="add_giftname" name="giftname"
									class="form-control" maxlength="50"></td>
								<td rowspan="7" class="col-xs-7"><input
									class="btn btn-default" type="file" id="photofile"
									onchange="showPic()"> <input class="btn btn-primary"
									type="button" value="上传图片" onclick="uploadFile()">&nbsp;<br>
									...
									</div> <img class="img-rounded" alt="暂无图片" id="img" width="150px"
									height="145px"></td>
							</tr>

							<tr>
								<td>礼品类型<font color=red>*</font></td>
								<td><input type="text" id="add_gifttype" name="gifttype"
							...
							<tr>
								...
								<td><input type="text" id="add_giftunit" name="giftunit"
									class="form-control" maxlength="20"></td>
							</tr>
							<tr>
								<td>礼品单价<font color=red>*</font></td>
								<td><input type="number" id="add_giftprice"
									name="giftprice" class="form-control" maxlength="6"></td>
							</tr>
							<tr>
								<td>所需积分<font color=red>*</font></td>
								<td><input type="number" id="add_integral" name="integral"
									class="form-control" maxlength="6"></td>
							</tr>
							<tr>
								<td>库存数量<font color=red>*</font></td>
								<td><input type="number" id="add_inventory"
									name="inventory" class="form-control" maxlength="8"></td>
							</tr>

							<tr id="tr8">
								<td>备注</td>
								<td colspan="2"><textarea class="form-control" rows="2"
										id='add_remake' name="remake" maxlength="500">
							  </textarea> <input type="hidden" name="lowernumber" value="0"></td>
							</tr>
						</table>
					</form>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal"
						id="add_gift_close_btn">关闭</button>
					<button type="button" class="btn btn-primary"
						id="add_gift_save_btn">保存</button>
				</div>
			</div>
			<!-- /.modal-content -->
		</div>
					...
					<form id="update_gift_form">
						<table class="table-striped table-bordered text-center">
							<tr>
								...
								<td><input type="text" class="form-control" name="giftname"
									id="giftname" required="required"></td>
								<td rowspan="9" class="col-xs-7"><input
									class="btn btn-default" type="file" id="photofile1"
									onchange="showPic1()"> <input class="btn btn-primary"
									type="button" value="上传图片" onclick="uploadFile1()">&nbsp;<br>
									...
									</div> <img class="img-rounded" id="img1" width="150px"
									height="145px"></td>
							</tr>
							<tr>
								<td>礼品编号</td>
							...
							<tr>
								<td>礼品类型</td>
								<td><input type="text" class="form-control" id="gifttype"
									name="gifttype" required="required"></td>
							</tr>
							<tr>
								<td>礼品状态</td>
								<td><select class="form-control" name="giftstate">
							...
							<tr>
								<td>礼品单位</td>
								<td><input type="text" class="form-control" name="giftunit"
									id="giftunit" required="required"></td>
							</tr>
							<tr>
								<td>礼品单价</td>
								<td><input type="text" class="form-control"
									name="giftprice" id="giftprice" required="required"></td>
							</tr>
							<tr>
								<td>所需积分</td>
								<td><input type="number" class="form-control"
									name="integral" id="integral" required="required"></td>
							</tr>
							<tr>
								<td>库存数量</td>
								<td><input type="number" class="form-control"
									name="inventory" id="inventory" required="required"></td>
							</tr>
							<tr>
								<td>兑换数量</td>
								<td><input type="number" class="form-control"
									name="lowernumber" id="lowernumber" required="required">
								</td>
							</tr>
							<tr id="tr10">
								<td>备注</td>
								<td colspan="2"><textarea class="form-control" rows="2"
										name="remake" id="remake" required="required"></textarea> <input
									type="hidden" name="id" id="giftId"></td>
							</tr>
						</table>
					</form>
				</div>
	...
	<script type="text/javascript">
		...
		function validate_add_from() {
			...
			if (!regx1.test(giftname)) {
				layer.msg("礼品名称：礼品名称必须是1-50个中文字符、英文和数字的组合");
				return false;
			}
			//礼品类型
			...
			if (!regx2.test(gifttype)) {
				layer.msg("礼品类型：礼品类型必须是1-30个中文字符、英文和数字的组合");
				return false;
			}
			//礼品单位
			...
			if (!giftunit.length > 20 || giftunit.length < 1) {
				layer.msg("礼品单位：礼品单位长度不能超过20个字符或不能为空！");
				return false;
			}
			//礼品单价
			...
			if (!regx4.test(giftprice)) {
				layer.msg("礼品单价：礼品单价必须是1-6位纯数字或不能为空！");
				return false;
			}
			//所需积分
			...
			if (!regx5.test(integral)) {
				layer.msg("所需积分：所需积分必须是1-6位纯数字或不能为空！");
				return false;
			}
			//库存数量
			...
			if (!regx6.test(inventory)) {
				layer.msg("库存数量：库存数量必须是1-6位纯数字或不能为空！");
				return false;
			}
			//备注信息
			var remake = $("#add_remake").val();
			if (remake.length > 500) {
				layer.msg("备注信息：备注信息长度不能超过500个字符");
				return false;
			}
		...
		function validate_update_from() {
			...
			if (!regx7.test(lowernumber)) {
				layer.msg("兑换数量：兑换数量必须是1-6位正数字或不能为空！");
				return false;
			}
	...
	<script type="text/javascript">
		...
		function removeImg(imgName) {
			$.ajax({
				url : "${APP_PATH}/giftmanagement/checkImg.action?isImg="
						+ imgName,
				type : "post",
									...
									.ajax({
										...
										success : function(result) {
											if (result.code == 100) {
												...
												window.location.href = "${APP_PATH}/giftmanagement/queryAllGift.action?pn="
														+ pages + 1;
												layer.msg("添加成功！");
											} else {
												var isImg = $("#image").val();
												if (isImg != undefined
														&& isImg != '') {//如果图片已上传，那么关闭之前将图片删除
													$("#image").remove();
													removeImg(isImg);
												}

												layer.msg("数据校验不成功，添加失败");
											}
		...
		$(document).on(
				...
				function() {
					...
					$.ajax({
						url : "${APP_PATH}/giftmanagement/findGift.action?id="
								+ id,
						type : "POST",
						success : function(result) {
							...
							if (g.image != null && g.image != '') {
								$("#img1").attr(
										"src",
										"${APP_PATH}/upload/oldmanback/accountmanage/giftmanage/"
												+ g.image);
							} else {
								$("#img1").removeAttr("src");
							}
						...
						function() {
							...
							$
									.ajax({
										...
										type : "POST",
										data : $("#update_gift_form")
												.serialize(),
										success : function(result) {
											if (result.code == 100) {
												var isImg = $("#image1").val();
												if (isImg != undefined
														&& isImg != '') {//如果图片已上传，那么关闭之前将图片文本删除
													$("#image1").remove();
												}
												window.location.href = "${APP_PATH}/giftmanagement/queryAllGift.action?pn="
														+ currentPage;
							...
							layer
									.confirm(
											...
											function(index) {
												$
														.ajax({
															url : "${APP_PATH}/giftmanagement/deleteGift/"
																	+ id,
															type : "delete",
															success : function(
																	result) {
