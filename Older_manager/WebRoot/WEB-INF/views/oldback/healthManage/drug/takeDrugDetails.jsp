		...
		<div class="row form-inline" align="left" style="margin-top:10px;">
			<div class="col-md-12">
				老人姓名： <input type="text" class="form-control" id="oldmanname1"
					...
					class="form-control" id="drugname1" placeholder="请输入药品名称"> 
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
			...
			<div class="col-md-12">
			   <form id="formid">
				<table class="table table-bordered table-hover text-center">
					...
					<tr>
					    <td>备注</td>
					    <td colspan="5"><textarea class="form-control" rows="2" cols="" name="takedrugdetails1"></textarea></td>
					</tr>
        ...
        <button type="button" class="btn btn-primary" id="save_drug_btn">保存</button>
      </div>
    </div>
  </div>
</div>
			...
			<div class="col-md-12">
			   <form id="form_update">
				<table class="table table-bordered table-hover text-center">
					...
					<tr>
					    <td>备注</td>
					    <td colspan="5"><textarea class="form-control" rows="2" cols="" name="takedrugdetails1" id="takedrugdetails1id"></textarea></td>
					</tr>
...
<div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" id="select_modal">
  <div class="modal-dialog modal-lg" role="document">
			   ...
			   <form id="form_update">
				<table class="table table-bordered table-hover text-center">
					<tr>
					     <td>老人名字</td>
					     <td>  
					         <p class="form-control-static" id="oldmanselect11"></p>
					     </td>
					     <td>药物名称</td>
					      <td>
					       <p class="form-control-static" id="medicinesid1"></p>
					     </td>
					     <td>开始服用时间</td>
					     <td>
					      <p class="form-control-static" id="starttakingdateid1"></p>
					     </td>
					</tr>
					<tr>
					     <td>服用结束时间</td>
					     <td>
					     <p class="form-control-static" id="endtakingdateid1"></p>
					     </td>
					     <td>用药剂量</td>
					      <td colspan="1">
					      <p class="form-control-static" id="dosageid1"></p>
					     </td>
					     <td colspan="2"></td>
					...
					<tr>
					   <td>使用方法</td>
					     <td> <p class="form-control-static" id="methoduseid1"></p></td>
					   <td>作用</td>
					   <td colspan="2"><p class="form-control-static" id="roleid1"></p> </td>
					</tr>
					<tr>
					    <td>备注</td>
					    <td colspan="5">
					    <p class="form-control-static" id="takedrugdetails1id1"></p>
					   </td>
					</tr>
	...
	var totalRecord, currentNum, oldmanname="",drugname="";
	/* 分页 */
	$(function() {
		go(1, "", "");
	...
	function build_page_nav(result) {
		...
		lastPageLi.click(function() {
			go(result.extend.pageInfo.pages, oldmanname,drugname);
		});
	...
	function build_table_data(result) {
		...
		var Td3 = $("<td></td>").append("药物名称");
		var Td4 = $("<td></td>").append("开始服用时间");
		var Td5 = $("<td></td>").append("服用结束时间");
		var Td6 = $("<td></td>").append("服用量");
		...
		$
				.each(
						...
						function(index, item) {
							...
							var ty3 = $("<td></td>").append(item.medicines);
							var ty4 = $("<td></td>").append(ChangeDateFormat(item.starttakingdate));
							var ty5 = $("<td></td>").append(ChangeDateFormat(item.endtakingdate));
							var ty6 = $("<td></td>").append(item.dosage);
		...
		$(document).on(
				...
				function() {
					...
					if (confirm("确认删除【" + oldername + "】吗")) {
						$.ajax({
							url : "${APP_PATH}/drug/deletetakedrugdetails/"+ delid,
							type : "DELETE",
							success : function(result) {
		...
		$("#old_delete_all_btn").click(
				function() {
					...
					if (confirm("确认删除【" + empNames + "】吗")) {
						//发送ajax请求
						$.ajax({
							url : "${APP_PATH}/drug/deletealldrug/"
									+ del_idstr,
							type : "DELETE",
							success : function(result) {
								//alert(result.msg);
     ...
     $.ajax({
           ...
           success:function(result){
              $.each(result.extend.oldman,function(){
						...
						//这个通过模态框的形式  模态框的id
						optionoldmanid.appendTo("#oldmanselect");
					});
           }
          });
...
//编辑信息按钮，弹出模态框
  $(document).on("click",".edit_btn",function(){
	  ...
	  var edit=$(this).attr("edit-id");
	   //3.把老人的id传递给模态框的编辑按钮
		$("#update_drug_btn").attr("edit-id",$(this).attr("edit-id"));
		
		

	  //发送请求获取
	  $.ajax({
			url:"${APP_PATH}/drug/selecttakedrugdetailsbyid/"+edit,
			type:"GET",
			...
			success:function(result){
				...
				if(result.code==100){
					...
					//console.log(result);
					var rel=result.extend.drug;
					$("#oldmanselect1").text(rel.oldman.name);
					 //alert(rel.oldman.name);
					$("#medicinesid").val(rel.medicines);
					$("#starttakingdateid").val(ChangeDateFormat(rel.starttakingdate));
					$("#endtakingdateid").val(ChangeDateFormat(rel.endtakingdate));
					$("#dosageid").val(rel.dosage);
					$("#methoduseid").val(rel.methoduse);
					$("#roleid").val(rel.role);
					$("#takedrugdetails1id").val(rel.takedrugdetails1);
					if(ChangeDateFormat(rel.endtakingdate)!=null&&ChangeDateFormat(rel.endtakingdate)!=null){
					   $("#update_modal").modal();
					}
	...
	//发送请求获取
	  ...
	  //发送请求获取
	  $.ajax({
			url:"${APP_PATH}/drug/selecttakedrugdetailsbyid/"+serch,
			type:"GET",
			...
			success:function(result){
				...
				if(result.code==100){
					...
					//console.log(result);
					var rel=result.extend.drug;
					$("#oldmanselect11").text(rel.oldman.name);
					 //alert(rel.oldman.name);
					$("#medicinesid1").text(rel.medicines);
					$("#starttakingdateid1").text(ChangeDateFormat(rel.starttakingdate));
					$("#endtakingdateid1").text(ChangeDateFormat(rel.endtakingdate));
					$("#dosageid1").text(rel.dosage);
					$("#methoduseid1").text(rel.methoduse);
					$("#roleid1").text(rel.role);
					$("#takedrugdetails1id1").text(rel.takedrugdetails1);
					if(ChangeDateFormat(rel.endtakingdate)!=null&&ChangeDateFormat(rel.endtakingdate)!=null){
					   $("#select_modal").modal();
					}
