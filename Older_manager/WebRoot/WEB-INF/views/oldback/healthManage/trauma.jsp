  ...
  <body>
   ...
   <!--路劲导航  -->
    ...
    <div class="col-md-12">
              ...
              外伤名称：<input type="text" class="form-control"  id="title1" placeholder="请输入活动名称">
   ...
   <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        ...
	        <h4 class="modal-title">新增外伤信息</h4>
	      </div>
	      <div class="modal-body">	
	           <form id="form_add">        
	            <table class="table table-bordered table-hover">
	               ...
	               <tr>
	                  <td >备注</td >
	                  <td colspan="3"><textarea name="remark" rows="2" cols="" class="form-control"></textarea></td>
	               </tr>
	      ...
	      <div class="modal-footer">
	        ...
	        <button type="button" class="btn btn-primary" id="save_trauma_btn">保存</button>
	      </div>
	    </div><!-- /.modal-content -->
	</div><!-- /.modal-dialog -->
</div><!-- /.modal -->
...
<div class="modal fade" tabindex="-1" role="dialog" id="update_modal">
   <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
	        <h4 class="modal-title">修改外伤信息</h4>
	      </div>
	      <div class="modal-body">	
	           <form class="" id="update_form">        
			    <div class="">
	            <table class="table table-bordered table-hover">
	               ...
	               <tr>
	                  <td >备注</td >
	                  <td colspan="3"><textarea name="remark" rows="2" cols="" class="form-control" id="remarkid"></textarea></td>
	               </tr>
	      ...
	      <div class="modal-footer">
	        <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
	        <button type="button" class="btn btn-primary" id="update_trauma_btn">更新</button>
	      </div>
	    </div><!-- /.modal-content -->
	</div><!-- /.modal-dialog -->
</div><!-- /.modal -->
...
<div class="modal fade" tabindex="-1" role="dialog" id="select_modal">
   <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
	        <h4 class="modal-title">查看外伤信息</h4>
	      </div>
	      <div class="modal-body">	
	           <form class="" id="update_form">        
			    <div class="">
	            <table class="table table-bordered table-hover">
	                ...
	                <tr>
	                 ...
	                 <td>外伤名称</td>
	                  <td>
	                    <p class="form-control-static" id="traumanameid1"> </p>
	                 </td>
	               </tr>
	               <tr>
	                 ...
	                 <td>治疗时间</td>
	                 <td>
	                   <p class="form-control-static" id="occurrencetimeid1"> </p>
	                 <td>医院名称</td>
	                  <td>
	               ...
	               <tr>
	                  <td >备注</td >
	                  <td colspan="3">
	                   <p class="form-control-static" id="remarkid1"> </p>
	                 </td>
	               </tr>
...
var totalRecord, currentNum,oldmanname="", traumaname="";
	/* 分页 */
	$(function() {
		go(1, "", "");
		$("#old_delete_all_btn").attr("disabled", true);
	});
	function go(pn,oldmanname,traumaname ) {
		$("#old_delete_all_btn").attr("disabled", true);
		layer.msg('数据加载中...', {
				icon : 16,
				shade : 0.01
		...
		$.ajax({
			url : "${APP_PATH}/trauma/selectalltrauma",
			data : {
				"pn" : pn,
	...
	function build_page_nav(result) {
		...
		if (result.extend.pageInfo.hasPreviousPage == false) {
			...
			prePageLi
					.click(function() {
						go(result.extend.pageInfo.pageNum - 1, oldmanname,traumaname);
					});
		}
		...
		lastPageLi.click(function() {
			go(result.extend.pageInfo.pages, oldmanname,traumaname);
		});
	...
	function build_table_data(result) {
		...
		var Td3 = $("<td></td>").append("外伤名称");
		var Td4 = $("<td></td>").append("治疗时间");
		...
		$
				.each(
						...
						function(index, item) {
							...
							var ty3 = $("<td></td>").append(item.traumaname);
							var ty4 = $("<td></td>").append(ChangeDateFormat(item.occurrencetime));
		...
		$(document).on(
				...
				function() {
					...
					if (confirm("确认删除【" + oldername + "】吗")) {
						$.ajax({
							url : "${APP_PATH}/trauma/deletetrauma/"+ delid,
							type : "DELETE",
							success : function(result) {
		...
		$("#old_delete_all_btn").click(
				function() {
					...
					if (confirm("确认删除【" + empNames + "】吗")) {
						//发送ajax请求
						$.ajax({
							url : "${APP_PATH}/trauma/deletealltrauma/"
									+ del_idstr,
							type : "DELETE",
							success : function(result) {
								//alert(result.msg);
 ...
 //点击编辑
  $(document).on("click",".edit_btn",function(){
	  ...
	  var edit=$(this).attr("edit-id");
	   //3.把老人的id传递给模态框的编辑按钮
		$("#update_trauma_btn").attr("edit-id",$(this).attr("edit-id"));
		
		

	  //发送请求获取
	  $.ajax({
			url:"${APP_PATH}/trauma/selecttraumabyid/"+edit,
			type:"GET",
			...
			success:function(result){
				...
				if(result.code==100){
					...
					//console.log(result);
					var rel=result.extend.trauma;
					$("#update_name").text(rel.oldman.name);
					 //alert(rel.oldman.name);
					$("#traumanameid").val(rel.traumaname);
					$("#occurrencetimeid").val(ChangeDateFormat(rel.occurrencetime));
 ...
 //点击查看
  $(document).on("click",".serchBtn",function(){
	...
	var serch=$(this).attr("serch-id");
	 //发送请求获取
	  $.ajax({
			url:"${APP_PATH}/trauma/selecttraumabyid/"+serch,
			type:"GET",
			...
			success:function(result){
				...
				if(result.code==100){
					...
					$("#select_name").text(rel.oldman.name);
					 //alert(rel.oldman.name);
					$("#traumanameid1").text(rel.traumaname);
					$("#occurrencetimeid1").text(ChangeDateFormat(rel.occurrencetime));
