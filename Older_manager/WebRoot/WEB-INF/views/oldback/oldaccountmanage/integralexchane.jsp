...
<body style="margin: 15px;">
		...
		<div class="row" style="margin-top:5px">
			<div class="col-md-12">
				<form class="form-inline"
					action="${APP_PATH}/integral/integralexchane" method="post">
					<div class="form-group">
						<label>&nbsp;&nbsp;老人姓名</label> <input type="text"
							name="oldman.name" class="form-control" placeholder="请输入老人姓名">
					</div>
					<div class="form-group">
						<label>变动时间</label> <input type="date" name="changetime1"
							class="form-control">
					</div>
					<div class="form-group">
						<label>到</label> <input type="date" name="changetime2"
							class="form-control">
					</div>
						...
						<tbody>
							<c:forEach items="${pageInfo.list }" var="in">
								<tr>
									...
									<td>${in.operator}</td>
									<td><fmt:formatDate value="${in.changetime}"
				...
				<nav aria-label="Page navigation">
					<ul class="pagination">
						...
						<c:if test="${pageInfo.hasPreviousPage==true}">
							<li><a
								href="${APP_PATH}/integral/integralexchane.action?pn=${1}">首页</a>
							</li>
							<li><a
								href="${APP_PATH}/integral/integralexchane.action?pn=${pageInfo.pageNum-1}">
						...
						<c:forEach items="${pageInfo.navigatepageNums}" var="p">
							<c:if test="${p==pageInfo.pageNum}">
								<li class="active"><a
									href="${APP_PATH}/integral/integralexchane.action?pn=${pageInfo.pageNum}">${p}</a></li>
							</c:if>
							<c:if test="${p!=pageInfo.pageNum}">
								<li><a
									href="${APP_PATH}/integral/integralexchane.action?pn=${p}">${p}</a></li>
							</c:if>
						</c:forEach>
						...
						<c:if test="${pageInfo.hasNextPage!=false}">
							...
							<li><a
								href="${APP_PATH}/integral/integralexchane.action?pn=${pageInfo.pages}">末页</a>
							</li>
						</c:if>
					</ul>
				</nav>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary"
						id="save_integral_btn">确定</button>
				</div>
			</div>
			<!-- /.modal-content -->
		</div>
	...
	<script type="text/javascript">
		...
		$(document)
				.on(
						...
						function() {
							...
							layer
									.confirm(
											"您确定要删除老人【" + name + "】的积分信息吗？",
											function(index) {
												$
														.ajax({
															url : "${APP_PATH}/integral/deleteIntegral/"
																	+ id,
															type : "delete",
															success : function(
																	result) {
					...
					.each(
							...
							function(index, items) {
								...
								tr.append(td1).append(td2).append(td3).append(
										td4).append(td5).appendTo(
										$("#add_oldman_table_body"));
							});
		}
		//构建模态框分页信息
		function build_nav_page(result) {
			...
			var li1 = $("<li class='previous'></li>")
					.append(
							$("<a></a>").append(
									$("<span>&larr;</span>").attr(
											"aria-hidden", "true")).append(
									"上一页"));
			var li2 = $("<li class='next'></li>")
					.append(
							$("<a></a>").append(
									$("<span>&rarr;</span>").attr(
											"aria-hidden", "true")).append(
		...
		$("#save_integral_btn")
				.click(
						function() {
							...
							if (ids.length < 1) {
								layer.msg("您还没选择要添加的老人！");
								return false;
							}
							$
									.ajax({
										url : "${APP_PATH}/integral/batchAddIntegral.action?ids="
												+ ids,
										type : "POST",
										success : function(result) {
											if (result.code == 100) {
