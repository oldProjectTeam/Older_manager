...
<body>
											...
											<li class="td td-item">
												...
												<div class="item-info">
													<div class="item-basic-info">
														<a
															href="${APP_PATH}/product/getProduct/${cart.product.id}"
															class="item-title J_MakePoint" value="${cart.id}"
															title="${cart.product.name}" data-point="tbcart.8.11">${cart.product.name}</a>
													</div>
												</div>
											</li>
	...
	<script type="text/javascript">
		...
		function submitOrder(userId) {
			...
			window.location.href = "${APP_PATH}/pay/orderPay.action?shippingaddressId="
					+ addressId
					+ "&formulaway="
					+ formulaWay
					+ "&paytype="
