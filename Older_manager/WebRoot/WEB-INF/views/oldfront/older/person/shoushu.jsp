				...
				<div class="col-md-9" style="background:#fff;min-height:647px;">
					...
					<div class="jksj" style="margin-top:15px;">
											...
											<div class="col-md-6 col-md-offset-2">
												<form class="form-horizontal " role="form" id="add_model_form">
													<div class="form-group">
														...
														<span style="color:red;">*</span>手术名称：</label>
														<div class="col-md-8" style="padding:0;">
															<input type="text" class="form-control" 
																name="surgeryname" id="surgeryname">																 
													...
													<div class="form-group">
														<label for="" class="col-md-4 control-label">
														<span style="color:red;">*</span>手术时间：</label>
														<div class="col-md-5" style="padding:0;">
															<input class="easyui-datebox datebox-f combo-f textbox-f"
																editable="false" id="surgerytime" name="surgerytime"
																style="width:180px; height: 30px; display: none;">
														</div>
														<div class="col-md-3">
						...
						<table
							...
							<thead>
								<tr class="xy-head">
									<th class="col-md-2">手术名称</th>
									<th class="col-md-2">手术时间</th>
									<th class="col-md-2">医院名称</th>
									<th class="col-md-3">备注</th>
									<th class="col-md-3">操作</th>
								</tr>
							</thead>
							<tbody id="surgery_tbody">
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">编辑手术数据</h4>
			</div>
			<div class="modal-body">
				<div class="row">
					<div class="col-md-6 col-md-offset-2">
						<form class="form-horizontal " role="form" id="edit_model_form">
							<div class="form-group">
								...
								<span style="color:red;">*</span>手术名称：</label>
								<div class="col-md-8" style="padding:0;">
									<input type="text" class="form-control" name="surgeryname" id="surgeryname1">																 
									<span style="color:red;"></span>
							...
							<div class="form-group">
								<label for="" class="col-md-4 control-label">医院名称：</label>
								<div class="col-md-8" style="padding:0;">
									<input type="text" class="form-control" id="hospitalname1"
										  name="hospitalname">
									<span style="color:red;"></span>
								</div>
							</div>
							<div class="form-group">
								...
								<div class="col-md-5" style="padding:0;">
									<input class="easyui-datebox datebox-f combo-f textbox-f"
										editable="false" id="surgerytime1" name="surgerytime"
										style="width:180px; height: 30px; display: none;">
								</div>
								<div class="col-md-3">
...
$("#add_save_btn").click(function(){
	...
	if(surgeryname==''){
		$("#surgeryname").next("span").html("请输入手术名称!");
		return false;
	}else if(!regex.test(surgeryname)){
		$("#surgeryname").next("span").html("手术名称可以是1-50的中文、数字、英文组合");
		return false;
	}else{
		$("#surgeryname").next("span").html("");
	}
	...
	var surgerytime=$("#surgerytime").datebox('getValue');
	if(surgerytime==''){
	...
	layer.msg('正在保存', {
	 ...
	 $.ajax({
		 url:"${APP_PATH}/surgery/insertSurgery",
...
function build_table(result){
	...
	$.each(result.extend.pageInfo.list,function(index,item){
		var td1=$("<td></td>").append(item.surgeryname);
		var td2=$("<td></td>").append(ChangeDateFormat(item.surgerytime));
		...
		var tr=$("<tr></tr>");
		tr.append(td1).append(td2).append(td3).append(td4).append(td5).appendTo("#surgery_tbody");
	});
}
...
function to_page(pn){
	...
	$.ajax({
		url:"${APP_PATH}/surgery/findAll",
		data:{
			pn:pn,
			oldmanId:'${older.oldmanId}',
			surgerytime1:$('#h_Time1').datebox('getValue'),
			surgerytime2:$('#h_Time2').datebox('getValue')
		},
		type:"post",
		success:function(result){
			layer.closeAll();
	 ...
	 function del(id) {
	     $.messager.confirm('删除记录信息', '您确定要删除该记录吗?', function (r) {
	         if (r) {
	             ...
	             $.ajax({
	                 type:'POST',
	                 url:'${APP_PATH}/surgery/deleteSurgery/'+id,	                       
	                 success: function (result) { 
	                	 layer.closeAll();
	                     if (result.code==100) {
	...
	function getSurger(id){
		...
		$.ajax({
			url:"${APP_PATH}/surgery/findSurgery/"+id,
			type:"POST",
			success:function(result){
				 
				layer.closeAll();
				if(result.code==100){
					var item=result.extend.surgery;
					$("#surgeryname1").val(item.surgeryname);
					$("#hospitalname1").val(item.hospitalname);
					$("#surgerytime1").datebox('setValue',ChangeDateFormat1(item.surgerytime));
					$("#remark1").val(item.remark);
					$("#surgeryId").val(item.id);
				}else{
					layer.msg("获取数据失败了",{
						icon:5,
						time:1000,
	...
	$("#edit_save_btn").click(function(){
		...
		var surgeryname=$("#surgeryname1").val();
		var regex = /^([\u4e00-\u9fa5]|[a-zA-Z0-9]){1,50}$/;
		if(surgeryname==''){
			$("#surgeryname1").next("span").html("请输入手术名称!");
			return false;
		}else if(!regex.test(surgeryname)){
			$("#surgeryname1").next("span").html("手术名称可以是1-50的中文、数字、英文组合");
			return false;
		}else{
			$("#surgeryname1").next("span").html("");
		}
		var hospitalname=$("#hospitalname1").val();
		if(hospitalname.length>100){
			$("#hospitalname1").next("span").html("医院长度不可超过100");
			return false;
		}else{
			$("#hospitalname1").next("span").html("");
		}
		var surgerytime=$("#surgerytime1").datebox('getValue');
		if(surgerytime==''){
		...
		layer.msg('修改中', {
		 ...
		 $.ajax({
			 url:"${APP_PATH}/surgery/updateSurgery",
	...
	$("#search_btn").click(function(){
		...
		$.ajax({
			url:"${APP_PATH}/surgery/findAll?"+time,
			data:{
				oldmanId:'${older.oldmanId}'
			},
			type:"post",
