...
public class CommonCodecUtils {
	...
	public static String getEntireFileSha1(String filePath) throws Exception {
		...
		try {
			...
			String errMsg = "getFileSha1 occur a exception, file:" + filePath
	...
	public static String getSlicePartSha1(String localPath, int sliceSize,
		...
		try {
			...
			int sliceCount = new Long((fileLength + (sliceSize - 1))
					/ sliceSize).intValue();
			...
			for (int sliceIndex = 0; sliceIndex < sliceCount - 1; ++sliceIndex) {
				...
				sliceJson.put(RequestBodyKey.UploadParts.DATA_SHA,
						sha1Utils.dumpTempState());
			...
			System.out
					.println("getSlicePartSha1 occur a error, filePath:{}, sliceSize:{}, exception:{}");
			throw e;
		} finally {
	...
	public static String getSlicePartSha1(byte[] contentBuffer, int sliceSize,
		...
		try {
			...
			System.out
					.println("getSlicePartSha1 from buffer occur a error, sliceSize:{}, exception:{}");
			throw e;
		} finally {
			if (fileInput != null) {
				try {
	...
	public static byte[] HmacSha1(byte[] binaryData, String key)
		...
		try {
			...
			System.out.println("mac not find algorithm {}");
			throw e;
		} catch (InvalidKeyException e) {
			System.out.println("mac init key {} occur a error {}");
			throw e;
		} catch (IllegalStateException e) {
			System.out.println("mac.doFinal occur a error {}");
			throw e;
		}
	}
