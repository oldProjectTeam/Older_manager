...
<form id="add_brand_form">
	<div class=" clearfix">
			...
			<div class="left_add">
				...
				<ul class="add_conent">
					<li id="firstLi" class=" clearfix"><label class="label_name"><i>*</i>品牌名称：</label>
						<input id="name" name="name" type="text" class="add_text" />
						<font color="red"></font>
					</li>
					<li class=" clearfix"><label class="label_name"><i>*</i>品牌序号：</label>
						<input id="sort" name="sort" type="number" class="add_text" style="width:80px" />
						<font color="red"></font>
					</li>
					<li class=" clearfix"><label class="label_name">品牌图片：</label>
					   <div class="row">
					   		<div class="col-sm-8">
					   		 <input type="file" class="form-control"
					   		   id="photofile" onchange="showPic()">
					   		</div>
					   		<div class="col-sm-2">
					   			<input class="btn btn-success" type="button" 
					   			value="上传图片" onclick="uploadFile()">
					   		</div>
					   </div>
					   ...
					   <div class="row">
					   		<div class="col-sm-5 col-md-offset-1">
					   		 <img class="img-rounded" src="${APP_PATH}/static/shop/images/image.png" id="img" width="150" height="150">
					   		</div>
					   		<div class="col-sm-4">
					...
					<li class=" clearfix"><label class="label_name"><i>*</i>所属地区：</label>
						<input id="region" name="region" type="text" class="add_text" style="width:120px" />
						<font color="red"></font>
					</li>
					...
					<li class=" clearfix"><label class="label_name"><i>*</i>显示状态：</label>
						<label>
						  <input name="state" type="radio" class="ace" value="1" checked="checked" />
							<span class="lbl">显示</span>
						</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<label>
						   <input type="radio" class="ace" name="state" value="0" />
						   <span class="lbl">不显示</span></label>
				    </li>
				</ul>
		...
		<div class="button_brand">
			<input id="add_brand_save_btn" type="button" class="btn btn-warning" value="保存" /><input
				name="" type="button" value="取消" id="control" class="btn btn-warning" />
		</div>
	</div>
    </form>
	<script type="text/javascript">
	   //保存品牌信息
	   $("#add_brand_save_btn").click(function(){
		   var name=$("#name").val();
		   if(name.length<1){
			   layer.msg('品牌名称不能为空！', {icon: 5,time : 1000,});
			   $("#name").next("font").html("品牌名称不能为空！");
			   return false;
		   }else if(name.length>50){
			   layer.msg('品牌名称长度不能超过50！', {icon: 5,time : 1000,});
		   ...
		   var sort=$("#sort").val();
		   if(sort==''||sort<0){
			   layer.msg('排序不能为空且不能为负数！', {icon: 5,time : 1000,});
		   ...
		   if(region.length<1){
			   layer.msg('地区不能为空！', {icon: 5,time : 1000,});
			   $("#region").next("font").html("地区不能为空！");
			   return false;
		   }else if(region.length>100){
			   layer.msg('地区不能超过100！', {icon: 5,time : 1000,});
		   ...
		   if(remark.length<1){
			   layer.msg('品牌描述不能为空！', {icon: 5,time : 1000,});
			   $("#des_msg").html("品牌描述不能为空！");
			   return false;
		   }else if(remark.length>500){
				layer.msg('描述长度不能超过500！', {icon: 5,time : 1000,});
