  ...
  <body>
   ...
   <!--路劲导航  -->
    ...
    <div class="col-md-12">
             ...
             遗传病名：<input type="text" class="form-control"  id="title1" placeholder="请输入活动名称">
   ...
   <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        ...
	        <h4 class="modal-title">新增感染信息</h4>
	      </div>
	      ...
	      <div class="modal-footer">
	        ...
	        <button type="button" class="btn btn-primary" id="save_genetichistory_btn">保存</button>
	      </div>
	    </div><!-- /.modal-content -->
	</div><!-- /.modal-dialog -->
</div><!-- /.modal -->
...
<div class="modal fade" tabindex="-1" role="dialog" id="update_modal">
   <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
	        <h4 class="modal-title">修改感染信息</h4>
	      </div>
	      <div class="modal-body">	
	           <form class="" id="add_activity_form">        
			    <div class="">
	            <table class="table table-bordered table-hover">
	      ...
	      <div class="modal-footer">
	        <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
	        <button type="button" class="btn btn-primary" id="update_genetichistory_btn">更新</button>
	      </div>
	    </div><!-- /.modal-content -->
	</div><!-- /.modal-dialog -->
</div><!-- /.modal -->
...
<div class="modal fade" tabindex="-1" role="dialog" id="select_modal">
   <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
	        <h4 class="modal-title">查看遗传信息</h4>
	      </div>
	      <div class="modal-body">	
	           <form class="" id="add_activity_form">        
			    <div class="">
	            <table class="table table-bordered table-hover">
	              ...
	              <tr>
	               ...
	               <tr>
	                 ...
	                 <td>确认时间</td>
	                 <td><p class="form-control-static" id="infectiontimeid1"></p></td>
	                 <td>确认医院</td>
	                  <td>
...
var totalRecord, currentNum,oldmanname="", diseasename="";
	...
	function go(pn,oldmanname,diseasename ) {
		$("#old_delete_all_btn").attr("disabled", true);
		$.ajax({
			url : "${APP_PATH}/history/selectallgenetichistory",
			data : {
				"pn" : pn,
	...
	function build_table_data(result) {
		...
		var Td3 = $("<td></td>").append("遗传病名");
		var Td4 = $("<td></td>").append("确认感染时间");
		var Td5 = $("<td></td>").append("确认感染地点");
		...
		$
				.each(
						...
						function(index, item) {
							...
							var ty4 = $("<td></td>").append(ChangeDateFormat(item.infectiontime));
		...
		$(document).on(
				...
				function() {
					...
					if (confirm("确认删除【" + oldername + "】吗")) {
						$.ajax({
							url : "${APP_PATH}/history/deletgenetichistory/"+ delid,
							type : "DELETE",
							success : function(result) {
		...
		$("#old_delete_all_btn").click(
				function() {
					...
					if (confirm("确认删除【" + empNames + "】吗")) {
						//发送ajax请求
						$.ajax({
							url : "${APP_PATH}/history/deleteallgenetichistory/"
									+ del_idstr,
							type : "DELETE",
							success : function(result) {
								//alert(result.msg);
  ...
  $(document).on("click",".edit_btn",function(){
	  ...
	  var edit=$(this).attr("edit-id");
	   //3.把老人的id传递给模态框的编辑按钮
		$("#update_genetichistory_btn").attr("edit-id",$(this).attr("edit-id"));
		
		

	  //发送请求获取
	  $.ajax({
			url:"${APP_PATH}/history/selectgenetichistory/"+edit,
			type:"GET",
			...
			success:function(result){
				...
				if(result.code==100){
					...
					//console.log(result);
					var rel=result.extend.history;
					$("#update_name").text(rel.oldman.name);
					$("#diseasenameid").val(rel.diseasename);
					$("#infectiontimeid").val(ChangeDateFormat(rel.infectiontime));
  ...
  $(document).on("click",".serchBtn",function(){
	...
	var serch=$(this).attr("serch-id");					
	  ...
	  //发送请求获取
	  $.ajax({
			url:"${APP_PATH}/history/selectgenetichistory/"+serch,
			type:"GET",
			...
			success:function(result){
				...
				if(result.code==100){
					...
					$("#infectiontimeid1").text(ChangeDateFormat(rel.infectiontime));
