				...
				<div class="col-md-9" style="background:#fff;min-height:647px;">
                    ...
                    <!-- 健康数据开始 -->
                      ...
                      <button type="button" class="btn back fr"onClick="javascript:history.go(-1);">返回</button>
                        ...
                        <form class="form-inline" role="form">
                            <div class="form-group col-md-5">
                                <label for="" class="control-label">开始时间：</label>
                                  <input class="easyui-datebox datebox-f combo-f textbox-f" editable="false" id="h_Time1" style="width: 180px; height: 30px; display: none;"> 
                            </div>
                            <div class="form-group col-md-5">
                                <label for="" class="control-label">结束时间：</label>
                                  <input class="easyui-datebox datebox-f combo-f textbox-f" editable="false" id="h_Time2" style="width: 180px; height: 30px; display: none;"> 
                            </div>
                        ...
                        <!-- 添加数据按钮开始 -->
                        <button type="button" class="tjsj" data-toggle="modal" data-target="#jkda">+添加数据录入</button>
                        <!-- 弹出框开始 -->
                        <div id="jkda" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="display: none;">
                                                ...
                                                <form class="form-horizontal " role="form" id="add_modal_form">
                                                    <div class="form-group">
                                                        <label for="" class="col-md-4 control-label">
                                                        	<span style="color:red;">*</span>输血原因：</label>
                                                        <div class="col-md-8" style="padding:0;">
                                                            <input type="text" class="form-control" 
																name="bloodtransfusionwhy" id="bloodtransfusionwhy">
                                                    ...
                                                    <div class="form-group">
                                                        <label for="" class="col-md-4 control-label">
                                                        <span style="color:red;">*</span>输血量：</label>
                                                        <div class="col-md-7" style="padding:0;">
                                                            <input type="text" class="form-control" name="bloodtransfusionamount"
                                                            id="bloodtransfusionamount">&nbsp;
                                                            <span style="color:red;"></span>
                                                        </div>
                                                    ...
                                                    <div class="form-group">
                                                        <label for="" class="col-md-4 control-label">
														<span style="color:red;">*</span>输血时间：</label>
                        ...
                        <!-- 添加数据按钮结束 -->
                        <!-- 列表开始 -->
                        <table class="table table-striped table-bordered table-hover text-center yyjl-table">
                            <thead>
                                <tr class="xy-head">
                                    <th class="col-md-2">输血原因</th>
                                    <th class="col-md-2">输血时间</th>
                                    <th class="col-md-2">输血量(ML)</th>
                                    <th class="col-md-3">备注</th>
                                    <th class="col-md-3">操作</th>
                                </tr>
                            </thead>
                            <tbody id="bloodtransfusion_tbody">
     ...
     <div class="modal-dialog modal-lg">
         <div class="modal-content">
             <div class="modal-header">
                 ...
                 <h4 class="modal-title">编辑输血数据</h4>
             </div>
             <div class="modal-body">
                 <div class="row">
                     <div class="col-md-6 col-md-offset-2">
                         <form class="form-horizontal " role="form" id="edit_modal_form">
                             <div class="form-group">
                                 ...
                                 <div class="col-md-8" style="padding:0;">
                                     <input type="text" class="form-control" 
										name="bloodtransfusionwhy" id="bloodtransfusionwhy1">
										 <span style="color:red;"></span>
										<input type="hidden" name="id" id="bloodtransfusionId"/>
                                 </div>
                             </div>
                             <div class="form-group">
                                 <label for="" class="col-md-4 control-label">
...
$("#add_save_btn").click(function(){
	...
	var bloodtransfusionwhy=$("#bloodtransfusionwhy").val();
	var regex = /^([\u4e00-\u9fa5]|[a-zA-Z0-9]){1,100}$/;
	if(bloodtransfusionwhy==''){
		$("#bloodtransfusionwhy").next("span").html("请输入输血原因!");
		return false;
	}else if(!regex.test(bloodtransfusionwhy)){
		$("#bloodtransfusionwhy").next("span").html("输血原因可以是1-100的中文、数字、英文组合");
		return false;
	}else{
		$("#bloodtransfusionwhy").next("span").html("");
	}
	 
	var bloodtransfusionamount=$("#bloodtransfusionamount").val();
	 var regex1=/^(([0-9]+\.[0-9]*[1-9][0-9]*)|([0-9]*[1-9][0-9]*\.[0-9]+)|([0-9]*[1-9][0-9]*))$/ ;
	if(bloodtransfusionamount==''){
		$("#bloodtransfusionamount").next("span").html("请输入输血量!");
		return false;
	}else if(!regex1.test(bloodtransfusionamount)){
		$("#bloodtransfusionamount").next("span").html("请输入正确的输血量");
		return false;
	}else{
		$("#bloodtransfusionamount").next("span").html("");
	}
	...
	layer.msg('正在保存', {
	 ...
	 $.ajax({
		 url:"${APP_PATH}/bloodtransfusion/insertBloodtransfusion",
...
function build_table(result){
	...
	$("#bloodtransfusion_tbody").empty();
	$.each(result.extend.pageInfo.list,function(index,item){
		var td1=$("<td></td>").append(item.bloodtransfusionwhy);
		var td2=$("<td></td>").append(ChangeDateFormat(item.occurrencetime));
		var td3=$("<td></td>").append(item.bloodtransfusionamount);
		...
		var tr=$("<tr></tr>");
		tr.append(td1).append(td2).append(td3).append(td4).append(td5).appendTo("#bloodtransfusion_tbody");
	});
}
...
function to_page(pn){
	...
	$.ajax({
		url:"${APP_PATH}/bloodtransfusion/findAll",
		data:{
			pn:pn,
	 ...
	 function del(id) {
	     $.messager.confirm('删除记录信息', '您确定要删除该记录吗?', function (r) {
	         if (r) {
	             ...
	             $.ajax({
	                 type:'POST',
	                 url:'${APP_PATH}/bloodtransfusion/deleteBloodtransfusion/'+id,	                       
	                 success: function (result) { 
	                	 layer.closeAll();
	                     if (result.code==100) {
	...
	//获得疾病信息
	function getBloodtransfusion(id){
		$("#edit_modal").modal();
		layer.msg("加载中",{
			icon:16,
			shade:0.01,
		...
		$.ajax({
			url:"${APP_PATH}/bloodtransfusion/findBloodtransfusion/"+id,
			type:"POST",
			success:function(result){
				 
				layer.closeAll();
				if(result.code==100){
					var item=result.extend.bloodtransfusion;
					$("#bloodtransfusionwhy1").val(item.bloodtransfusionwhy);
					$("#bloodtransfusionamount1").val(item.bloodtransfusionamount);
					$("#occurrencetime1").datebox('setValue',ChangeDateFormat1(item.occurrencetime));
					$("#remark1").val(item.remark);
					$("#bloodtransfusionId").val(item.id);
				}else{
					layer.msg("获取数据失败了",{
						icon:5,
						time:1000,
	...
	$("#edit_save_btn").click(function(){
		...
		var bloodtransfusionwhy=$("#bloodtransfusionwhy1").val();
		var regex = /^([\u4e00-\u9fa5]|[a-zA-Z0-9]){1,100}$/;
		if(bloodtransfusionwhy==''){
			$("#bloodtransfusionwhy1").next("span").html("请输入输血原因!");
			return false;
		}else if(!regex.test(bloodtransfusionwhy)){
			$("#bloodtransfusionwhy1").next("span").html("输血原因可以是1-100的中文、数字、英文组合");
			return false;
		}else{
			$("#bloodtransfusionwhy1").next("span").html("");
		}
		 
		var bloodtransfusionamount=$("#bloodtransfusionamount1").val();
		 var regex1=/^(([0-9]+\.[0-9]*[1-9][0-9]*)|([0-9]*[1-9][0-9]*\.[0-9]+)|([0-9]*[1-9][0-9]*))$/ ;
		if(bloodtransfusionamount==''){
			$("#bloodtransfusionamount1").next("span").html("请输入输血量!");
			return false;
		}else if(!regex1.test(bloodtransfusionamount)){
			$("#bloodtransfusionamount1").next("span").html("请输入正确的输血量");
			return false;
		}else{
			$("#bloodtransfusionamount1").next("span").html("");
		}
		...
		if(remark.length>200){
			$("#remar1k").next("span").html("备注长度不可超过200");
			return false;
		}else{
		...
		layer.msg('修改中', {
		 ...
		 $.ajax({
			 url:"${APP_PATH}/bloodtransfusion/updateBloodtransfusion",
	...
	$("#search_btn").click(function(){
		...
		$.ajax({
			url:"${APP_PATH}/bloodtransfusion/findAll?"+time,
			data:{
				oldmanId:'${older.oldmanId}'
			},
			type:"post",
