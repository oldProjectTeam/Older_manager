			   ...
			   <form id="formid">
				<table class="table table-bordered table-hover text-center">
					<tr>
	                 <td>老人姓名</td>
	                 <td>
	                   <select id="oldman_name" class="form-control" name="oldman.id">
	                 		</select>
	                 </td>
	                 <td>体检医院</td>
	                  <td>
                         <input name="peinfo.hospital" type="text" class="form-control">
	                 </td>
	               </tr>
	               <tr>
	                 
	                 <td>体检时间</td>
	                 <td><input name="peinfo.petime" type="date" class="form-control"></td>
	                 <td>下次体检时间</td>
	                  <td>
                        <input name="peinfo.nexttime" type="date" class="form-control">
	                 </td>
	               </tr>
	                <tr>
	                 
	                 <td>体检项目</td>
	                 <td><textarea class="form-control" rows="2" name="peproject"></textarea></td>
	                 <td>体检值</td>
	                  <td>
                        <textarea class="form-control" rows="2" name="pevalue"></textarea>
	                 </td>
	               </tr>
	               <tr>
	                 
	                 <td>体检结果</td>
	                 <td><textarea class="form-control" rows="2" name="peresult"></textarea></td>
	                 <td>结果分析</td>
	                  <td>
                        <textarea class="form-control" rows="2" name="resultanalysis"></textarea>
	                 </td>
	               </tr>
	               <tr>
	                  <td >备注</td >
	                  <td colspan="3"><textarea class="form-control" rows="2" name="peinfo.remark"></textarea></td>
	               </tr>
			   ...
			   <form id="form_update">
				<table class="table table-bordered table-hover text-center">
					<tr>
	                 ...
	                 <td>体检医院</td>
	                  <td>
                         <input name="peinfo.hospital" type="text" class="form-control"  id="hospitalid">
	                 </td>
	               </tr>
	               <tr>
	                 
	                 <td>体检时间</td>
	                 <td><input name="peinfo.petime" type="date" class="form-control" id="petimeid"></td>
	                 <td>下次体检时间</td>
	                  <td>
                        <input name="peinfo.nexttime" type="date" class="form-control" id="nexttimeid">
	                 </td>
	               </tr>
	                <tr>
	                 
	                 <td>体检项目</td>
	                 <td><textarea class="form-control" rows="2" name="peproject" id="peprojectid"></textarea></td>
	                 <td>体检值</td>
	                  <td>
                        <textarea class="form-control" rows="2" name="pevalue" id="pevalueid"></textarea>
	                 </td>
	               </tr>
	               <tr>
	                 
	                 <td>体检结果</td>
	                 <td><textarea class="form-control" rows="2" name="peresult" id="peresultid"></textarea></td>
	                 <td>结果分析</td>
	                  <td>
                        <textarea class="form-control" rows="2" name="resultanalysis" id="resultanalysisid"></textarea>
	                 </td>
	               </tr>
	               <tr>
	                  <td >备注</td >
	                  <td colspan="3"><textarea class="form-control" rows="2" name="peinfo.remark" id="remarkid"></textarea></td>
	               </tr>
  ...
  <div class="modal-dialog modal-lg" role="document">
				...
				<table class="table table-bordered table-hover text-center">
	               ...
	               <tr>
	                ...
	                <tr>
	                 ...
	                 <td>体检项目</td>
	                 <td> <p class="form-control-static" id="peprojectid1"></p></td>
	                 <td>体检值</td>
	                  <td>
	...
	function build_page_nav(result) {
		...
		if (result.extend.pageInfo.hasPreviousPage == false) {
			...
			prePageLi
					.click(function() {
						go(result.extend.pageInfo.pageNum - 1, oldmanname,hospitalname);
					});
		}
		...
		if (result.extend.pageInfo.hasNextPage == false) {
			...
			nextPageLi
					.click(function() {
						go(result.extend.pageInfo.pageNum + 1, noldmanname,hospitalname);
					});
		}
		//添加首页和前一页的提示
		...
		$
				.each(
						...
						function(index, item) {
							...
							var ty3 = $("<td></td>").append(item.peinfo.hospital);
							var ty4 = $("<td></td>").append(ChangeDateFormat(item.peinfo.petime));
							var ty5 = $("<td></td>").append(ChangeDateFormat(item.peinfo.nexttime));
...
//编辑信息按钮，弹出模态框
  $(document).on("click",".edit_btn",function(){
	  ...
	  var edit=$(this).attr("edit-id");
	   //3.把老人的id传递给模态框的编辑按钮
		$("#update_pe_btn").attr("edit-id",$(this).attr("edit-id"));
	  ...
	  $.ajax({
			...
			success:function(result){
				...
				if(result.code==100){
					...
					$("#petimeid").val(ChangeDateFormat(rel.peinfo.petime));
					$("#nexttimeid").val(ChangeDateFormat(rel.peinfo.nexttime));
	  ...
	  $.ajax({
			...
			success:function(result){
				...
				if(result.code==100){
					...
					$("#petimeid1").text(ChangeDateFormat(rel.peinfo.petime));
					$("#nexttimeid1").text(ChangeDateFormat(rel.peinfo.nexttime));
					$("#peprojectid1").text(rel.peproject);
					$("#pevalueid1").text(rel.pevalue);
					$("#peresultid1").text(rel.peresult);
					$("#resultanalysisid1").text(rel.resultanalysis);
