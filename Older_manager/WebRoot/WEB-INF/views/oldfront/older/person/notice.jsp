...
function build_list(result){
	...
	$.each(result.extend.pageInfo.list,function(index,item){
		 ...
		 var a1211=$("<a></a>").attr("href","${APP_PATH}/older/notice_detail/"+item.id)
		 .append(item.content);
		 div121.append(a1211);
		 div12.append(div121);
...
function buil_list_pageInfo(result){
	...
	if(result.extend.pageInfo.hasPreviousPage==false){
		...
		prePage.click(function(){
			load_notice(result.extend.pageInfo.pageNum-1);
		});
	}
	if(result.extend.pageInfo.hasNextPage==false){
		...
		lastPage.click(function(){
			load_notice(result.extend.pageInfo.pages);
		});
	}
...
function load_notice(pn){
	...
	$("#search_btn").click(function(){
		...
		$.ajax({
			url:"${APP_PATH}/notice/findAll?"+time,	 
			type:"post",
			success:function(result){
				layer.closeAll();
				//1.构建表格数据
