 ...
 <script src="${APP_PATH}/static/bootstrap-3.3.7-dist/js/bootstrap.min.js"></script>
  ...
  <body>
   ...
   <!--路劲导航  -->
    ...
    <div class="col-md-12">
              老人姓名： <input type="text" class="form-control"  id="oldmanname1" placeholder="请输入老人名字">
              健康状况：<select id="health1" class="form-control">
                    <option>请选择</option>
                    <option>健康</option>
                    <option>亚健康</option>
                    <option>不健康</option>
			   ...
			   <form id="formid">
				<table class="table table-bordered table-hover text-center">
	               ...
	               <tr>
	                 ...
	                 <td>健康评估</td>
	                  <td>
                        <textarea class="form-control" rows="2" name="healthdataassessment"></textarea>
	                 </td>
	               </tr>
	                     <tr>
	                  <td >医生建议</td >
	                  <td colspan="3"><textarea class="form-control" rows="2" name="suggest"></textarea></td>
	               </tr>
...
<div class="modal fade" tabindex="-1" role="dialog" id="update_modal">
   <div class="modal-dialog" role="document">
	    <div class="modal-content">
			   ...
			   <form id="form_update">
				<table class="table table-bordered table-hover text-center">
					<tr>
	                 ...
	                 <td>医生姓名</td>
	                  <td>
                         <input name="doctor" type="text" class="form-control" id="doctorid">
	                 </td>
	               </tr>
	               <tr>
	                 
	                 <td>健康状况</td>
	                 <td><textarea class="form-control" rows="2" name="health" id="healthid"></textarea></td>
	                 <td>健康评估</td>
	                  <td>
                        <textarea class="form-control" rows="2" name="healthdataassessment" id="healthdataassessmentid"></textarea>
	                 </td>
	               </tr>
	                     <tr>
	                  <td >医生建议</td >
	                  <td colspan="3"><textarea class="form-control" rows="2" name="suggest" id="suggestid"></textarea></td>
	               </tr>
	      ...
	      <div class="modal-footer">
	        <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
	        <button type="button" class="btn btn-primary" id="update_assess_btn">更新</button>
	      </div>
	    </div><!-- /.modal-content -->
	</div><!-- /.modal-dialog -->
</div><!-- /.modal -->
			   ...
			   <form id="">
				<table class="table table-bordered table-hover text-center">
	               ...
	               <tr>
	                 ...
	                 <td>健康状况</td>
	                 <td>
	                  <p class="form-control-static" id="healthid1"></p></td>
	                 <td>健康评估</td>
	                  <td>
	                  ...
	                  <td >医生建议</td >
	                  <td colspan="3">
	                   <p class="form-control-static" id="suggestid1"></td>
	               </tr>
...
var totalRecord, currentNum, oldmanname="",healthname="";
	...
	function build_page_nav(result) {
		...
		if (result.extend.pageInfo.hasPreviousPage == false) {
			...
			prePageLi
					.click(function() {
						go(result.extend.pageInfo.pageNum - 1, oldmanname,healthname);
					});
		}
		...
		$
				.each(
						...
						function(index, item) {
							...
							var ty5 = $("<td></td>").append(item.healthdataassessment);
...
//编辑信息按钮，弹出模态框
  $(document).on("click",".edit_btn",function(){
	  ...
	  var edit=$(this).attr("edit-id");
	   //3.把老人的id传递给模态框的编辑按钮
		$("#update_assess_btn").attr("edit-id",$(this).attr("edit-id"));
		
		

	  //发送请求获取
	  $.ajax({
			url:"${APP_PATH}/access/selecthealthassessbyid/"+edit,
			type:"GET",
			...
			success:function(result){
				...
				if(result.code==100){
					...
					$("#healthdataassessmentid").val(rel.healthdataassessment);
  ...
  $(document).on("click",".serchBtn",function(){
	var serch=$(this).attr("serch-id");
	 //发送请求获取
	  $.ajax({
			url:"${APP_PATH}/access/selecthealthassessbyid/"+serch,
			type:"GET",
			...
			success:function(result){
				...
				if(result.code==100){
					...
					$("#healthdataassessmentid1").text(rel.healthdataassessment);
