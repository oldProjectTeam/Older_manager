		...
		<div class="row" style="margin-top:5px">
			<div class="col-md-12">
				<form class="form-inline"
					action="${APP_PATH}/oldmanaccount/queryAllAccount.action"
					method="post">
					<div class="form-group">
						&nbsp;&nbsp;<label for="exampleInputName2">老人名字</label> <input
							type="text" class="form-control" name="oldman.name"
							placeholder="请输入老人名字">
					</div>
					<div class="form-group">
						<label for="exampleInputEmail2">登录账户</label> <input type="text"
							class="form-control" name="account" placeholder="请输入登录账户">
					</div>
						...
						<tbody>
							<c:forEach items="${pageInfo.list}" var="ac">
								<tr>
									...
									<td>
										<button type="button"
											class="btn btn-success btn-sm psrest_btn">
				...
				<nav aria-label="Page navigation">
					<ul class="pagination">
						...
						<c:if test="${pageInfo.hasPreviousPage==true}">
							<li><a
								href="${APP_PATH}/oldmanaccount/queryAllAccount.action?pn=${1}">首页</a>
							</li>
							<li><a
								href="${APP_PATH}/oldmanaccount/queryAllAccount.action?pn=${pageInfo.pageNum-1}">
						...
						<c:forEach items="${pageInfo.navigatepageNums}" var="p">
							<c:if test="${p==pageInfo.pageNum}">
								<li class="active"><a
									href="${APP_PATH}/oldmanaccount/queryAllAccount.action?pn=${pageInfo.pageNum}">${p}</a></li>
							</c:if>
							<c:if test="${p!=pageInfo.pageNum}">
								<li><a
									href="${APP_PATH}/oldmanaccount/queryAllAccount.action?pn=${p}">${p}</a></li>
							</c:if>
						</c:forEach>
						...
						<c:if test="${pageInfo.hasNextPage!=false}">
							...
							<li><a
								href="${APP_PATH}/oldmanaccount/queryAllAccount.action?pn=${pageInfo.pages}">末页</a>
							</li>
						</c:if>
					</ul>
				</nav>
		...
		$(document)
				.on(
						...
						function() {
							...
							layer
									.confirm(
											"您确定要删除老人【" + name + "】的账户信息吗？",
											function(index) {
												$
														.ajax({
															url : "${APP_PATH}/oldmanaccount/deleteAccount/"
																	+ id,
															type : "delete",
															success : function(
																	result) {
		...
		$("#add_account_btn")
				.click(
						function() {
							...
							$
									.ajax({
										url : "${APP_PATH}/oldmanaccount/batchAddAcount.action?ids="
												+ ids,
										type : "POST",
										success : function(result) {
											if (result.code == 100) {
