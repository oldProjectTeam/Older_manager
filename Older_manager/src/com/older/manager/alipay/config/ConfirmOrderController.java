...
public class ConfirmOrderController {
	...
	public ModelAndView confirmOrder(String ids, Integer userId,
			ModelAndView modal) throws NumberFormatException, Exception {
		// 待确认订单信息集合
		...
		if (ids != null && !"".equals(ids) && ids.contains("-")) {
			...
			for (String id : str) {
				if (!Pattern.matches("[0-9]+", id))
					continue;
				cart = shoppingCartService.selectWithProductById(Integer
						.parseInt(id));
				if (cart == null)
					continue;
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/now_buy/{id}&{number}&{userId}")
	public ModelAndView now_buy(@PathVariable("id") Integer id,
			@PathVariable("number") Integer number,
			@PathVariable("userId") Integer userId, ModelAndView modal)
	...
	public ModelAndView wait_buy(Integer id, ModelAndView modal)
			throws NumberFormatException, Exception {
