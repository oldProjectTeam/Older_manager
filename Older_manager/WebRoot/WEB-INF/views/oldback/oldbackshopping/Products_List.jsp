...
<body>
		...
		<div id="products_style">
			<div class="search_style">
				...
				<form id="select_produts_form" method="post">
					<label>产品名称</label> <input name="name" type="text" class="text_add"
						placeholder="输入品牌名称" style=" width:250px" /> <label>添加时间区为</label>
					<input class="inline laydate-icon " name="addtime" type="date"
						id="start" style=" margin-left:10px;"> <label>—></label> <input
						class="inline laydate-icon " name="addtime1" type="date"
			...
			<div class="border clearfix">
				<span class="l_f"> <a href="picture-add" title="添加商品"
					...
					<a class="btn btn-danger" id="delete_products_info"> <i
						class="icon-trash"></i>批量删除
				</a>
				</span> <span class="r_f" id="shopNum"></span>
			</div>
			<!--产品列表展示-->
						...
						<thead>
							<tr>
								<th width="25px"><label><input type="checkbox"
										class="all_check" class="ace"><span class="lbl"></span></label></th>
	...
	<script type="text/javascript">
			...
			$
					.each(
							...
							function(index, item) {
								...
								var proNum = $("<td></td>").append(item.number)
										.attr("state_num", item.state);
								var proName = $("<td></td>").append(item.name);
								var proOrPrice = $("<td></td>").append(
										item.orprice);
								var proNowPrice = $("<td></td>").append(
										item.nowprice);
								var proRegion = $("<td></td>").append(
										item.region);
								var proAddtime = $("<td></td>").append(
										item.addtime);
								var proAuditStatus = $("<td></td>").append(
										item.auditstatus == 1 ? "通过" : "等待审核");
								var proState = $("<td></td>").append(
										item.state == 1 ? "已启用" : "已停用");
								var startBtn = '';
								if (item.state == 1) {
									startBtn = $("<button title='停用'></button>")
											.addClass(
													"btn btn-success btn-sm block_btn")
											.append(
									...
									startBtn = $("<button title='启用'></button>")
											.addClass(
													"btn btn-default btn-sm block_btn")
											.append(
								...
								var updatBtn = $("<button title='编辑'></button>")
										.addClass(
												"btn btn-warning btn-sm compile_btn")
										.append(
												"<span class='icon-edit' aria-hidden='true'></span>");
								var deleteBtn = $(
										"<button title='删除'></button>")
										.addClass(
												"btn btn-danger btn-sm delet_produt_btn")
										.append(
												"<span class='icon-trash' aria-hidden='true'></span>");
								;
								var btntd = $("<td></td>").append(startBtn)
										.append(updatBtn).append(deleteBtn);
								$("<tr></tr>").append(checktd).append(proNum)
										.append(proName).append(proOrPrice)
										.append(proNowPrice).append(proRegion)
										.append(proAddtime).append(
												proAuditStatus)
										.append(proState).append(btntd)
		...
		function page_info(result) {
			$("#sample-table_info").empty();
			$("#shopNum").empty();
			$("#sample-table_info").append(
					"第&nbsp;&nbsp;" + result.extend.pageInfo.pageNum
							+ "&nbsp;&nbsp;页");
			$("#shopNum").append(
					"共：<b>" + result.extend.pageInfo.total + "</b>件商品");
		}
		//分页条
		function prod_page_num(result) {
			...
			pageUl.append(lastPrePage).append(lastPage).appendTo(
		...
		//批量删除
		$("#delete_products_info").click(
				function() {
					if ($(".item_check:checked").length != 0) {
						var prodName = "";
						var prodNumber = "";
						$.each($(".item_check:checked"), function() {
							prodName += $(this).parents("tr").find("td:eq(2)")
									.text()
									+ ",";
							prodNumber += $(this).parents("tr")
									.find("td:eq(1)").text()
									+ "-";
						});
						...
						if (confirm("你确定要删除{" + prodName + "}吗？")) {
							$.ajax({
								url : "${APP_PATH}/shopping/deleteProduct/"
										+ prodNumber,
								type : "DELETE",
								success : function(result) {
									if (result.code == 100) {
