...
<body style="margin: 15px;">
	...
	<div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog"
		aria-labelledby="myLargeModalLabel" id="recomactivity_modal">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal"
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
					<button type="button" class="btn btn-primary"
						id="recomactivities_save_btn">保存</button>
					<button type="button" class="btn btn-primary"
						id="recomactivities_btn">推荐</button>
				</div>
			</div>
			<!-- /.modal-content -->
		</div>
	...
	<script type="text/javascript">
			...
			$
					.each(
							...
							function(index, item) {
								...
								var recommandBtn2 = $("<button></button>")
										.addClass(
												"btn btn-info btn-sm recommandBtn2")
										.append(
												$("<span></span>")
														.addClass(
		...
		$(document).on(
				...
				function() {
					...
					$.ajax({
						...
						type : "GET",
						success : function(result) {
							$.each(result.extend.activities, function(index,
									item) {
								activity_result = result.extend.activities;
								var optionactivityid = $("<option></option>")
										.append(this.title).attr("value",
												item.id);
								optionactivityid.val(item.activitytime + ":"
										+ item.place + ":" + item.id);
								optionactivityid
										.appendTo("#recommand_activity_title");
		...
		//推荐点击保存 
		$("#recomactivities_save_btn")
				.click(
						function() {
							var edit = $(this).attr("edit-id");
							$
									.ajax({
										url : "${APP_PATH}/RecomActivityDetails/addRecomActivitydetails/"
												+ activityId,
										type : "POST",
										data : $("#add_recomactivity_form")
												.serialize(),
										success : function(result) {
