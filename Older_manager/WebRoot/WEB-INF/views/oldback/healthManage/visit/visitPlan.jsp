	...
	<div class="col-md-12">
		...
		<div class="row form-inline" align="left" style="margin-top:10px;">
			<div class="col-md-12" id="searchDiv">
				老人姓名：
				<input type="text" class="form-control" id="oldName" placeholder="请输入老人姓名">
				老人身份证：
				<input type="text" id="idcard" class="form-control" id="" placeholder="请输入老人身份证号码">
	...
	<script type="text/javascript">
        ...
        function build_table_data(result) {
            ...
            headTR.append(checkBox).append(Td1).append(Td2).append(Td3).append(Td4).append(Td5)
                    .append(Td6).append(Td7).append(Td8).appendTo($("#head"));

            $.each(result.extend.pageInfo.list, function(index, item) {
                var checkBoxTd = $("<td><input type='checkbox' class='check_item'/></td>");
                var id = $("<td></td>").append(item.id);
                var oldName = $("<td></td>").append(item.oldman.name);
                var oldID = $("<td></td>").append(item.oldman.idcar);
                var oldBackTime = $("<td></td>").append(ChangeDateFormat(item.visiplantime));
                var oldBackTraction = $("<td></td>").append(item.plannedvisits);
                var state = $("<td></td>").append(item.state);
                var createTime = $("<td></td>").append(ChangeDateFormat(item.createtime));
                ...
                var itemPlan = item.id + ":" + item.oldman.name + ":" + item.oldman.idcar;
                ...
                var btnTd = $("<td></td>").append(serchBtn).append(" ").append(delBtn);
                //append方法执行完成以后还是返回原来的元素							              								              
                $("<tr></tr>").append(checkBoxTd).append(id).append(oldName).append(oldID).append(
                        oldBackTime).append(oldBackTraction).append(state).append(createTime)
        ...
        $(document).on("click", ".serchBtn", function() {
            ...
            window.location.href = "${APP_PATH}/plan/selectvisitPlan/" + itemPlan;
        ...
        $("#old_delete_all_btn").click(function() {
            ...
            if (confirm("确认删除【" + empNames + "】吗")) {
                //发送ajax请求
                $.ajax({
                    url : "${APP_PATH}/plan/deleteAllVisitPlan/" + del_idstr,
                    type : "DELETE",
                    success : function(result) {
                        //回到当前页面
                        go(currentNum, strs);
