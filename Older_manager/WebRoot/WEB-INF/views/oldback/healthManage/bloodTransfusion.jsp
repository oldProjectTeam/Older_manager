   ...
   <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      ...
	      <div class="modal-footer">
	        ...
	        <button type="button" class="btn btn-primary" id="save_blood_btn">保存</button>
	      </div>
	    </div><!-- /.modal-content -->
	</div><!-- /.modal-dialog -->
</div><!-- /.modal -->
...
<div class="modal fade" tabindex="-1" role="dialog" id="update_modal">
   <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      ...
	      <div class="modal-body">	
	           <form  id="add_activity_form">        
	            ...
	            <table class="table table-bordered table-hover">
	               ...
	               <tr>
	                  <td >备注</td >
	                  <td colspan="3"><textarea name="remark" rows="2" cols="" class="form-control" id="remakid"></textarea></td>
	               </tr>
	      ...
	      <div class="modal-footer">
	        <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
	        <button type="button" class="btn btn-primary" id="update_blood_btn">更新</button>
	      </div>
	    </div><!-- /.modal-content -->
	</div><!-- /.modal-dialog -->
</div><!-- /.modal -->
...
<div class="modal fade" tabindex="-1" role="dialog" id="select_modal">
   <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
	        <h4 class="modal-title">查看输血信息</h4>
	      </div>
	      <div class="modal-body">	
	           <form  id="add_activity_form">        
			    <div class="">
	            <table class="table table-bordered table-hover">
	               ...
	               <tr>
	                 ...
	                 <td>输血原因</td>
	                  <td>
                         <p class="form-control-static" id="select_why"></p>
	                 </td>
	               </tr>
	               <tr>
	                 ...
	                 <td>输血量</td>
	                  <td>
                        <p class="form-control-static" id="select_liang"></p>
	                 </td>
	               </tr>
	               <tr>
	                  <td >备注</td >
	                  <td colspan="3"><p class="form-control-static" id="select_remark"></p></td>
	               </tr>
...
var totalRecord, currentNum,oldmanname="";
	...
	function go(pn,oldmanname ) {
		...
		$.ajax({
			url : "${APP_PATH}/blood/selectallblood",
			data : {
				"pn" : pn,
	...
	function build_table_data(result) {
		...
		var Td3 = $("<td></td>").append("输血原因");
		var Td4 = $("<td></td>").append("输血时间");
		var Td5 = $("<td></td>").append("输血量");
		...
		$
				.each(
						...
						function(index, item) {
							...
							var ty3 = $("<td></td>").append(item.bloodtransfusionwhy);
							var ty4 = $("<td></td>").append(ChangeDateFormat(item.occurrencetime));
							var ty5 = $("<td></td>").append(item.bloodtransfusionamount);
		...
		$(document).on(
				...
				function() {
					...
					if (confirm("确认删除【" + oldername + "】吗")) {
						$.ajax({
							url : "${APP_PATH}/blood/deletebloodbyid/"+ delid,
							type : "DELETE",
							success : function(result) {
		...
		$("#old_delete_all_btn").click(
				function() {
					...
					if (confirm("确认删除【" + empNames + "】吗")) {
						//发送ajax请求
						$.ajax({
							url : "${APP_PATH}/blood/deleteallblood/"
									+ del_idstr,
							type : "DELETE",
							success : function(result) {
								//alert(result.msg);
 ...
 //点击编辑
   $(document).on("click",".edit_btn",function(){
	  ...
	  var edit=$(this).attr("edit-id");
	   //3.把老人的id传递给模态框的编辑按钮
		$("#update_blood_btn").attr("edit-id",$(this).attr("edit-id"));
		
		

	  //发送请求获取
	  $.ajax({
			url:"${APP_PATH}/blood/selectbloodbyid/"+edit,
			type:"GET",
			...
			success:function(result){
				...
				if(result.code==100){
					...
					var rel=result.extend.blood;
					$("#update_name").text(rel.oldman.name);
					$("#bloodtransfusionwhyid").val(rel.bloodtransfusionwhy);
					$("#occurrencetimeid").val(ChangeDateFormat(rel.occurrencetime));
					$("#bloodtransfusionamountid").val(rel.bloodtransfusionamount);
  ...
  $(document).on("click",".serchBtn",function(){
	...
	var serch=$(this).attr("serch-id");
	  ...
	  //发送请求获取
	  $.ajax({
			url:"${APP_PATH}/blood/selectbloodbyid/"+serch,
			type:"GET",
			...
			success:function(result){
				...
				if(result.code==100){
					...
					$("#select_why").text(rel.bloodtransfusionwhy);
					$("#select_time").text(ChangeDateFormat(rel.occurrencetime));
					$("#select_liang").text(rel.bloodtransfusionamount);
					$("#select_remark").text(rel.remark);
